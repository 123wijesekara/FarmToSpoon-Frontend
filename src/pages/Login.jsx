  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Set cookie
        Cookies.set('userId', response.data.data.userId, { expires: 7, sameSite: 'Lax', secure: false });
      ...
      // Show SweetAlert if account is suspended/deactivated
      if (error.response?.data?.message?.includes("suspended") || error.response?.data?.message?.includes("deactivated")) {
        Swal.fire({
          icon: 'warning',
